...
dir <- "C:\\Users\\tgrosu\\OneDrive\\Desktop\\CutRun\\seacr_stringent_merged_from_peaks\\contrasts\\"
...
write.csv(annotated_peaks_WT_unique_ub, file = paste0(dir, "WT_unique_ub.csv"), quote = FALSE, row.names = FALSE)
write.csv(annotated_peaks_A7_unique_ub, file = paste0(dir, "A7_unique_ub.csv"), quote = FALSE, row.names = FALSE)
write.csv(annotated_peaks_C9_unique_ub, file = paste0(dir, "C9_unique_ub.csv"), quote = FALSE, row.names = FALSE)
write.csv(annotated_peaks_A7_C9_common_ub, file = paste0(dir, "A7_C9_common_ub.csv"), quote = FALSE, row.names = FALSE)
write.csv(annotated_peaks_WT_A7_C9_overlap_ub, file = paste0(dir, "WT_A7_C9_overlap_ub.csv"), quote = FALSE, row.names = FALSE)
...
summary_annotation_ub$Condition <- factor(summary_annotation_ub$Condition, 
                                           levels = c("WT_A7_C9_overlap_ub", "A7_C9_common_ub", "C9_unique_ub", "A7_unique_ub", "WT_unique_ub"))
...
WT_unique_ub_summary_perc <- WT_unique_ub_summary %>%
  dplyr::mutate(WT_unique_ub_percentage = WT_unique_ub/sum(WT_unique_ub_summary$WT_unique_ub)*100) %>%
  dplyr::select(-WT_unique_ub) 

A7_unique_ub_summary_perc <- A7_unique_ub_summary %>%
  dplyr::mutate(A7_unique_ub_percentage = A7_unique_ub/sum(A7_unique_ub_summary$A7_unique_ub)*100) %>%
  dplyr::select(-A7_unique_ub) 

C9_unique_ub_summary_perc <- C9_unique_ub_summary %>%
  dplyr::mutate(C9_unique_ub_percentage = C9_unique_ub/sum(C9_unique_ub_summary$C9_unique_ub)*100) %>%
  dplyr::select(-C9_unique_ub) 

A7_C9_common_ub_summary_perc <- A7_C9_common_ub_summary %>%
  dplyr::mutate(A7_C9_common_ub_percentage = A7_C9_common_ub/sum(A7_C9_common_ub_summary$A7_C9_common_ub)*100) %>%
  dplyr::select(-A7_C9_common_ub) 

WT_A7_C9_overlap_ub_summary_perc <- WT_A7_C9_overlap_ub_summary %>%
  dplyr::mutate(WT_A7_C9_overlap_ub_percentage = WT_A7_C9_overlap_ub/sum(WT_A7_C9_overlap_ub_summary$WT_A7_C9_overlap_ub)*100) %>%
...
ggsave(filename = "./CUT_and_RUN/plots/annotated_constrast_ub_peaks_percentage.pdf", 
       plot = annotated_peaks_plot_perc, 
       width = 38, height = 13, dpi = 800, units = "cm", device = cairo_pdf)



peak_width_plot <- annotated_peaks_WT_unique_ub %>%
  mutate(Condition = "WT") %>%
  rbind(mutate(annotated_peaks_A7_unique_ub, Condition = "A7")) %>%
  rbind(mutate(annotated_peaks_C9_unique_ub, Condition = "C9")) %>%
  ...
  scale_colour_manual(values = c("#8B008B", "#FFA500", "#000000")) +
  theme_classic(base_size = 26) +
  theme(legend.position = "none") +
  annotate("text", label = paste0(median(annotated_peaks_WT_unique_ub$width), "bp"), x = 1, y = 3.12, size = 5.5) +
  annotate("text", label = paste0(median(annotated_peaks_A7_unique_ub$width), "bp"), x = 2, y = 3.32, size = 5.5) +
  annotate("text", label = paste0(median(annotated_peaks_C9_unique_ub$width), "bp"), x = 3, y = 3.6, size = 5.5) + 
  xlab("Sample") +
  ylab("H2AK119ub peaks width\n(log10bp)")

ggsave(dir,filename = "ub_peak_width.pdf", 
