...
WT_counts_ub <- featureCounts("path\\merged_bams\\WT_ub_deduped.bam", # count reads
                               annot.inbuilt = "hg38",
...
A7_counts_ub <- featureCounts("path\\merged_bams\\A7_ub_deduped.bam", # count reads
                               annot.inbuilt = "hg38",
...
C9_counts_ub <- featureCounts("path\\merged_bams\\C9_ub_deduped.bam", # count reads
                               annot.inbuilt = "hg38",
...
ggsave(a7_ub_gene_expr, file = "path\\scatter_density_a7_ub.pdf", width = 7, height = 6, dpi = 300)
...
c9_ub_gene_expr <- ggplot(ub_and_rna, aes(x = logFC_C9_WT, y = LogFC)) +
  ...
  theme(axis.title = element_text(face = "bold", size = 12),
        plot.title = element_text(hjust = 0.5, face = "bold"),
        legend.position = "right")
ggsave(c9_ub_gene_expr, file = "path\\scatter_density_c9_ub.pdf", width = 7, height = 6, dpi = 300)
