...
WT_counts_ac <- featureCounts(".\\merged_bams\\WT_ac_deduped.bam", # count reads
  ...
  rename(c("GeneID" = "Var1", "WT_counts_ac" = "value")) %>%
  na.omit()
...
C9_counts_ac <- featureCounts(".\\merged_bams\\C9_ac_deduped.bam", # count reads
  ...
  melt() %>%
  dplyr::select(c("Var1", "value")) %>%
  rename(c("GeneID" = "Var1", "C9_counts_ac" = "value")) %>%
  na.omit()


C9_counts_ac$GeneID <- as.character(C9_counts_ac$GeneID)
A7_counts_ac$GeneID <- as.character(A7_counts_ac$GeneID)
WT_counts_ac$GeneID <- as.character(WT_counts_ac$GeneID)
...
counts_matrix <- full_join(WT_counts_ac, A7_counts_ac, by = "GeneID") %>%
  full_join(C9_counts_ac, by = "GeneID") %>%
  dplyr::left_join(gene_lengths, by = "GeneID")

counts_matrix$WT_counts_ac <- as.numeric(counts_matrix$WT_counts_ac)
counts_matrix$C9_counts_ac <- as.numeric(counts_matrix$C9_counts_ac)
counts_matrix$A7_counts_ac <- as.numeric(counts_matrix$A7_counts_ac)
...
counts_list <- DGEList(counts=counts_matrix[,c("WT_counts_ac", "A7_counts_ac", "C9_counts_ac")], genes=data.frame(Length=counts_matrix[,"geneLength"]))
...
RPKM$logFC_A7_WT <- RPKM$A7_counts_ac - RPKM$WT_counts_ac
RPKM$logFC_C9_WT <- RPKM$C9_counts_ac - RPKM$WT_counts_ac
...
ac_and_rna <- left_join(RPKM_A7, expression_wt_vs_a7, by = "geneID") %>%
  na.omit()

cor_results <- cor.test(ac_and_rna$logFC_A7_WT, ac_and_rna$LogFC, method = "pearson")


a7_ac_gene_expr <- ggplot(ac_and_rna, aes(x = logFC_A7_WT, y = LogFC)) +
  ...
  labs(x = "H3K27ac A7 - WT", y = "mRNA A7 - WT") +
...
ac_and_rna <- left_join(RPKM_C9, expression_wt_vs_c9, by = "geneID") %>%
  na.omit()

cor_results <- cor.test(ac_and_rna$logFC_C9_WT, ac_and_rna$LogFC, method = "pearson")


c9_ac_gene_expr <- ggplot(ac_and_rna, aes(x = logFC_C9_WT, y = LogFC)) +
  stat_density_2d(aes(fill = after_stat(density)),
                  geom = "raster", contour = FALSE) +
  ...
  geom_vline(xintercept = 0, linetype = "dashed",
             color = "black", size = 0.5) +
  scale_fill_gradient(low = "white", high = "black") +
  labs(x = "H3K27ac C9 - WT", y = "mRNA C9 - WT") +
  annotate("text", x = Inf, y = Inf, hjust = 1.1, vjust = 1.5,
           label = paste0("R = ", round(cor_results$estimate, 2),
                          "\np = ", format.pval(cor_results$p.value, digits = 2)),
           size = 4.5, color = "black") +
...
#this script uses output from reannotation_of_peaks.R script and selects the H3K27ac regions in promoters 
#ooutput: correlation analysis with gene expression
annotated_peaks_WT_ac_promoters <- annotated_peaks_WT_ac %>%
  dplyr::filter(annotation %in% c("Promoter (2-3kb)", "Promoter (1-2kb)", "Promoter (<=1kb)"))

annotated_peaks_A7_ac_promoters <- annotated_peaks_A7_ac %>%
  dplyr::filter(annotation %in% c("Promoter (2-3kb)", "Promoter (1-2kb)", "Promoter (<=1kb)"))

annotated_peaks_C9_ac_promoters <- annotated_peaks_C9_ac %>%
  dplyr::filter(annotation %in% c("Promoter (2-3kb)", "Promoter (1-2kb)", "Promoter (<=1kb)"))

annotated_peaks_wt_ac_promoters_saf <- annotated_peaks_WT_ac_promoters %>%
  dplyr::filter(transcriptBiotype == "protein_coding") %>%
  dplyr::select(c("SYMBOL", "seqnames", "start", "end", "strand")) %>%
  dplyr::rename(c("GeneID" = "SYMBOL", 
                  "Chr" = "seqnames",
...
annotated_peaks_a7_ac_promoters_saf <- annotated_peaks_A7_ac_promoters %>%
  dplyr::filter(transcriptBiotype == "protein_coding") %>%
  dplyr::select(c("SYMBOL", "seqnames", "start", "end", "strand")) %>%
  dplyr::rename(c("GeneID" = "SYMBOL", 
                  "Chr" = "seqnames",
...
annotated_peaks_c9_ac_promoters_saf <- annotated_peaks_C9_ac_promoters %>%
  dplyr::filter(transcriptBiotype == "protein_coding") %>%
  dplyr::select(c("SYMBOL", "seqnames", "start", "end", "strand")) %>%
  dplyr::rename(c("GeneID" = "SYMBOL", 
                  "Chr" = "seqnames",
...
gene_lengths <- annotated_peaks_WT_ac_promoters %>%
  rbind(annotated_peaks_A7_ac_promoters) %>%
  #dplyr::filter(transcriptBiotype == "protein_coding") %>%
  dplyr::select(c("SYMBOL", "geneLength")) %>%
  slice_max(geneLength, by = SYMBOL) %>%
  na.omit() %>%
...
RPKM_A7 <- RPKM[,-5] %>%
  dplyr::filter(geneID %in% intersect_me3_wt_ac_a7)
